...
import com.qianyan.search.express.query.ExpressQueryParser;
import com.qianyan.search.express.query.ExpressSearcher;
import com.qianyan.search.express.query.FilterFactory;
import com.qianyan.search.express.query.HighLighter;
import com.qianyan.search.express.query.KeywordFormat;
...
public class ExpressSearcherImpl implements ExpressSearcher {
     ...
     * @param docparser
     */
    public ExpressSearcherImpl(String[] indexdir, ExpressQueryParser parser,
                               DocumentParser docparser, HighLighter highlighter) {
    ...
    public List<ExpressDocument> search(SearchForm s, Navbar page) {
        ...
        if (s.getS() == 1) {
            ...
            if (s.getP() == 1 & s.getT() == 0) needredo = true;
        }
        try {
            ...
            if (filter == null) {
                ...
                if (needredo) hits2 = searcher.search(query, page.PAGESIZE);
            }
            else {
                hits = searcher.search(query, filter,page.PAGESIZE, sort);
                if (needredo) hits2 = searcher.search(query, filter2, page.PAGESIZE);
            }
